...
void VULKAN_SWAPCHAIN::INIT(WINDOW WIN, VULKAN_PHYSICAL_DEVICE PH_DEVICE, VULKAN_LOGICAL_DEVICE LG_DEVICE)
{
    ...
    vkGetSwapchainImagesKHR(LG_DEVICE.GET_HANDLE_TO_VK_LOGICAL_DEVICE(), this->SW_CHAIN, &imageCounter, nullptr);
    this->SW_CHAIN_IMAGES.resize(imageCounter);
    vkGetSwapchainImagesKHR(LG_DEVICE.GET_HANDLE_TO_VK_LOGICAL_DEVICE(), this->SW_CHAIN, &imageCounter, this->SW_CHAIN_IMAGES.data());


    this->SW_CHAIN_IMAGE_VIEWS.resize(this->SW_CHAIN_IMAGES.size());
    for(size_t i = 0; i < this->SW_CHAIN_IMAGES.size(); i++)
    {
        VkImageViewCreateInfo createInfo{};
        createInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
        createInfo.image = this->SW_CHAIN_IMAGES[i];
        createInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
        createInfo.format = this->SW_CHAIN_IMAGE_FORMAT;
...
{
    for(auto& imageView : this->SW_CHAIN_IMAGE_VIEWS)
    {
