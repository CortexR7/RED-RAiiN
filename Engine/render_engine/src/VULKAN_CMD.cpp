...
void VULKAN_CMD::RECORD_CMD_BUFFER_GRAPHICS(uint32_t CURRENT_FRAME, VkCommandBuffer& CMD_BUFFER, uint32_t imageIndex, VULKAN_SWAPCHAIN& SWAPCHAIN,  VULKAN_PIPELINE& PIPELINE, RE2_VERTEX_BUFFER& VERTEX_BUFFER, RE2_INDEX_BUFFER& INDEX_BUFFER, VULKAN_DESC_SET& DESC_SET)
{
    ...
    VkBuffer VK_BUFFERS[] = {VERTEX_BUFFER.BUFFER[0]};
    VkDeviceSize OFFSETS[] = {0};
    vkCmdBindVertexBuffers(CMD_BUFFER, 0, 1, VK_BUFFERS, OFFSETS);
    vkCmdBindIndexBuffer(CMD_BUFFER, INDEX_BUFFER.BUFFER[0], 0, VK_INDEX_TYPE_UINT16);
    vkCmdBindDescriptorSets(CMD_BUFFER, VK_PIPELINE_BIND_POINT_GRAPHICS, PIPELINE.PIPELINE_LAYOUT, 0, 1, &DESC_SET.DESCRIPTOR_SETS[CURRENT_FRAME], 0, nullptr);
    vkCmdDrawIndexed(CMD_BUFFER, static_cast<uint32_t>(INDEX_BUFFER.INDEX_COUNT[0]), 1, 0, 0, 0);
