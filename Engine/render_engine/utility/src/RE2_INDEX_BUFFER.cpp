...
void RE2_INDEX_BUFFER::INIT(void* DATA, size_t SIZE, VULKAN_LOGICAL_DEVICE LGD, VULKAN_PHYSICAL_DEVICE PHD, const VkCommandPool& CMD_POOL)
{
    RE2_VK_BUFFER::INIT(LGD, PHD, CMD_POOL);
    RE2_VK_BUFFER::BUFFER_SIZE.resize(1);
    RE2_VK_BUFFER::BUFFER_SIZE[0] = SIZE;
...
void RE2_INDEX_BUFFER::INIT(void* DATA, size_t SIZE, VULKAN_LOGICAL_DEVICE LGD, VULKAN_PHYSICAL_DEVICE PHD, const VkCommandPool& CMD_POOL, uint16_t INDEX_COUNT)
{
    this->INDEX_COUNT.resize(1);
    this->INDEX_COUNT[0] = INDEX_COUNT;
    this->INIT(
        DATA,
        SIZE,
        LGD,
...
void RE2_INDEX_BUFFER::COPY_BUFFER_DATA(VkBuffer SRC_BUFFER, VkBuffer DST_BUFFER, VkDeviceSize SIZE)
{
...
size_t RE2_INDEX_BUFFER::EXPOSE_SIZE(uint32_t n)
{
    return RE2_VK_BUFFER::BUFFER_SIZE[n];
}
